...
function MemberAvatar({
  ...
  display_name,
  size = "sm",
  borderClassName = "border-black/30",
}: {
  ...
  const sizeClassName = size === "md" ? "h-10 w-10" : "h-8 w-8";
  ...
  return (
    <div
      className={`${sizeClassName} flex items-center justify-center rounded-full border-2 ${borderClassName} bg-neutral-700 text-xs font-bold text-white`}
...
export default function CameraPageClient() {
  ...
  const [selectedRoom, setSelectedRoom] = useState<CameraRoom | null>(null);
  ...
  useEffect(() => {
    ...
    async function initialize() {
      try {
        const myRooms = await getMyCameraRooms();

        if (cancelled) return;

        setRooms(myRooms);
        setSelectedRoom(myRooms[0] ?? null);

        if (myRooms.length === 0) {
          setErrorMessage("참여 중인 방이 없습니다. 먼저 방에 참여해 주세요.");
          setStep("error");
          return;
        }
  ...
  }, [clearPreviewUrl, clearTimers, showError, startCamera, stopStream]);
  ...
  const handleUpload = async () => {
    if (!recordedBlob || !selectedRoom) {
      showError("업로드할 영상이나 방 정보가 없습니다.");
      return;
    }
  ...
  const selectedMembers = selectedRoom?.members.slice(0, 3) ?? [];
    ...
    <main className="relative min-h-[100dvh] overflow-hidden bg-black text-white">
      ...
      <header className="relative z-20 flex items-center justify-between px-5 pb-4 pt-[max(1.25rem,env(safe-area-inset-top))]">
        ...
        <button
          ...
          disabled={!selectedRoom || step === "recording" || step === "uploading"}
      ...
      {(step === "camera" || step === "recording") && selectedRoom && (
        <>
            ...
            <div className="rounded-full bg-black/20 px-4 py-2 text-center backdrop-blur-md">
              <p className="text-sm font-medium">
                {step === "recording"
                  ...
                  : selectedRoom.description || "오늘의 순간을 인증해 보세요"}
              </p>
            </div>
          </section>
      ...
      {step === "recording" && (
        <div className="absolute left-1/2 top-24 z-20 flex -translate-x-1/2 items-center gap-2 rounded-full bg-black/35 px-4 py-2 backdrop-blur-lg">
          ...
          <span className="text-xs font-semibold tracking-[0.16em]">RECORDING</span>
        </div>
      )}
      ...
      {step === "requesting" && (
        <div className="absolute inset-0 z-30 flex items-center justify-center bg-black px-6">
          <div className="text-center">
            ...
            <p className="mt-2 text-xs text-white/50">카메라와 마이크 권한을 허용해 주세요</p>
          </div>
        </div>
      )}
      ...
      {step === "error" && (
        <div className="absolute inset-0 z-40 flex items-center justify-center bg-black px-5">
          <div className="w-full max-w-sm rounded-[28px] bg-white p-6 text-black">
            <p className="text-xs font-semibold tracking-[0.16em] text-neutral-400">CAMERA ERROR</p>
            <h1 className="mt-2 text-2xl font-bold tracking-[-0.05em]">카메라를 열 수 없어요</h1>
      ...
      {(step === "preview" || step === "uploading") && selectedRoom && (
        <div className="absolute inset-0 z-20 flex flex-col justify-between bg-black/15">
          <div className="px-5 pt-[max(1.5rem,env(safe-area-inset-top))]">
            <div className="mx-auto flex w-fit items-center gap-2 rounded-full bg-black/30 px-4 py-2 backdrop-blur-lg">
              ...
              <span className="text-sm font-semibold">
                {step === "uploading" ? "인증을 보내고 있어요" : "3초 인증 준비 완료"}
              </span>
            </div>
          </div>
              ...
              <div className="mb-4 flex items-center justify-between px-1">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-white/55">방 멤버에게 인증을 요청합니다</p>
                  </div>
                </div>
        ...
        <div className="absolute inset-0 z-40 flex items-center justify-center bg-black/65 px-5 backdrop-blur-xl">
          <div className="w-full max-w-sm rounded-[32px] bg-white p-6 text-black">
            <div className="mb-7 flex items-start justify-between">
              <div>
                <p className="text-xs font-semibold tracking-[0.16em] text-neutral-400">CERTIFIED</p>
                <h1 className="mt-2 text-3xl font-bold tracking-[-0.06em]">인증을 보냈어요</h1>
              </div>
            ...
            <div className="rounded-3xl bg-[#f4f4f4] p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-neutral-500">멤버들의 인정을 기다리고 있어요</p>
                </div>
          ...
          <div className="mx-auto flex max-w-md items-center justify-between px-8">
            ...
            {step === "recording" ? (
              ...
              <button
                ...
                className="relative flex h-24 w-24 items-center justify-center rounded-full border border-white/45 disabled:opacity-40"
                aria-label="촬영 시작"
              >
        ...
        <>
          ...
          <section className="fixed bottom-0 left-0 right-0 z-50">
            <div className="mx-auto max-w-md rounded-t-[34px] bg-white px-5 pb-[max(2.5rem,env(safe-area-inset-bottom))] pt-4 text-black">
              ...
              <div className="mb-5">
                <p className="text-xs font-semibold tracking-[0.18em] text-neutral-400">SELECT ROOM</p>
                <h2 className="mt-1 text-2xl font-bold tracking-[-0.05em]">어디에 인증할까요?</h2>
              </div>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="flex -space-x-2">
                          {room.members.slice(0, 3).map((member) => (
                            <MemberAvatar
                              ...
                              display_name={member.display_name}
                              size="md"
                              borderClassName={active ? "border-black" : "border-[#f4f4f4]"}
                            />
                          ))}
                        </div>
                        ...
                        <div>
                          ...
                          <p className={`mt-0.5 text-xs ${active ? "text-white/55" : "text-neutral-400"}`}>
                            {room.description || "오늘의 인증 방"}
                          </p>
                        </div>
                      </div>

                      <span className={`h-3 w-3 rounded-full ${
                        active ? "bg-white" : "border border-neutral-300 bg-white"
                      }`} />
                    </button>
                  );
                })}
